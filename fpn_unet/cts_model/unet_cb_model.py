...
class DecoderBlock(nn.Module):
   def __init__(self, decoder_in_channels, out_channels, skip_channels=None,skip_struct=False, use_batchnorm=True, attention_type=None):
      super().__init__()
      ...
      self.skip_struct = skip_struct
      if skip_struct:
         self.skipconv = Conv2dReLU(skip_channels, decoder_in_channels, kernel_size=1, use_batchnorm=use_batchnorm)
...
class UnetDecoder(nn.Module):
   def __init__(self, encoder_channels, decoder_channels=(256, 128, 64, 32, 16), final_channels=1, use_batchnorm=True, center=False, attention_type=None):
      ...
      self.layer1 = DecoderBlock(in_channels[0], out_channels[0], encoder_channels[1], skip_struct=True, use_batchnorm=use_batchnorm, attention_type=attention_type)
      self.layer2 = DecoderBlock(in_channels[1], out_channels[1], encoder_channels[2], skip_struct=True, use_batchnorm=use_batchnorm, attention_type=attention_type)
      self.layer3 = DecoderBlock(in_channels[2], out_channels[2], encoder_channels[3], skip_struct=True, use_batchnorm=use_batchnorm, attention_type=attention_type)
      self.layer4 = DecoderBlock(in_channels[3], out_channels[3], encoder_channels[4], skip_struct=True, use_batchnorm=use_batchnorm, attention_type=attention_type)
